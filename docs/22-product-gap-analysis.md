...
|---|---|---|---|
| Control plane services | Cluster registry, pricing imports/lookups, query reads, recommendation workflow, in-process export job manifests, in-process tenant membership, in-process audit events, principal introspection, and in-process policy versioning now expose minimal tenant-scoped V1 APIs through the gateway. | Users cannot operate the full product lifecycle through the documented APIs, and tenant membership/audit/policy storage is not durable. | Implement OIDC-backed identity, durable tenant membership, durable audit storage, durable policy storage/events, durable export delivery, and richer workflow service surfaces. |
