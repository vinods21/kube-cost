...
|---|---|---|---|
| Control plane services | Cluster registry, pricing imports/lookups, query reads, recommendation workflow, in-process export job manifests, in-process tenant membership, in-process audit events, principal introspection, in-process policy versioning, and in-process integration configuration now expose minimal tenant-scoped V1 APIs through the gateway. | Users cannot operate the full product lifecycle through durable APIs, and tenant membership/audit/policy/integration storage is not durable. | Implement OIDC-backed identity, durable tenant membership, durable audit storage, durable policy storage/events, durable integration storage/validation, durable export delivery, and richer workflow service surfaces. |
| API authentication and tenancy | Gateway resolves static bearer token mappings to trusted tenant and principal headers, strips caller-supplied tenant/principal headers, and can enforce gateway-only shared secret plus HMAC-signed backend requests on fronted backends. Tenant service exposes in-process membership for the current tenant. Helm can render opt-in NetworkPolicy defaults for gateway/backend, agent/ingestion, and operator metrics ingress. OIDC/JWKS, durable membership authorization, and certificate-backed workload identity are not implemented. | Static token and shared-secret/HMAC mapping is still bootstrap identity, not production authn/authz. | Add OIDC/JWKS authn, durable tenant membership checks, and certificate-backed service identity. |
