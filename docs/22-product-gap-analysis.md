...
| Query and quality APIs | Query now exposes tenant-scoped `/api/v1/data-quality`, `/api/v1/usage`, `/api/v1/costs`, `/api/v1/allocation`, recommendation reads, bounded namespace/cluster/promoted-dimension grouping, opaque cursors, optional freshness/coverage summaries, and in-process async query jobs with checksummed inline manifests. Durable async job storage, arbitrary raw label grouping, and export-grade object-storage result manifests are not implemented. | Product can expose initial synchronous analysis and short-lived async jobs, but high-cardinality exploration and export-grade query workflows are still incomplete. | Add durable async job storage, cardinality-budgeted raw label grouping, and export/object-storage result manifests. |
