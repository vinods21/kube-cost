...
|---|---|---|---|
| Control plane services | Cluster registry, pricing imports/lookups, query reads, recommendation workflow, in-process export job manifests, in-process tenant membership, and in-process audit events now expose minimal tenant-scoped V1 APIs through the gateway. Identity mostly exposes health only. | Users cannot operate the full product lifecycle through the documented APIs, and tenant membership/audit storage is not durable. | Implement identity, durable tenant membership, durable audit storage, durable export delivery, and richer workflow service surfaces. |
