...
|---|---|---|---|
| Control plane services | Cluster registry, pricing imports/lookups, query reads, recommendation workflow, in-process export job manifests, and in-process tenant membership now expose minimal tenant-scoped V1 APIs through the gateway. Identity and audit mostly expose health only. | Users cannot operate the full product lifecycle through the documented APIs, and tenant membership is not durable. | Implement identity, durable tenant membership, audit, durable export delivery, and richer workflow service surfaces. |
